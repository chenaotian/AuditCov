...
- Web 和命令行均可创建、列出和删除代码仓项目，无需由 Agent 初始化项目。
- 命令行支持创建、列出、删除项目，并查询全部或指定会话的覆盖率。
- 全部会话的项目总覆盖率和文件最大单行读取次数随成功 Read 实时物化，常用总览查询无需重复扫描历史事件。
- 按项目、工具和原生会话 ID 分开统计覆盖率。
- 展示源码目录树、文件最大单行读取次数、逐行覆盖状态和重复读取热度。
- 可切换到分批加载的全部文件列表，按最大单行读取次数升序或降序排列。
...
2. **统计服务层**：匹配项目、冻结分母、关联调用前后事件、合并成功读取区间，并在同一 SQLite 事务中更新全部会话的覆盖总览和文件读取峰值。
...
其中 `1` 是 `auditcov project list` 显示的项目 ID。该命令默认直接读取已物化的全部会话轻量总览；需要完整目录树时使用：

```bash
auditcov coverage 1 --tree --json
```

`--tree` 决定是否请求完整目录树，`--json` 只决定输出格式。
...
- 文件名前的数字表示该文件中单行最高读取次数；未读取文件显示红色 `0`，正数越大绿色越深。
- 默认按可展开目录树浏览，也可切换到 **All files** 并按该数字升序或降序排列。
- 点击源码文件，查看成功读取过的行、未读取行和每行具体读取次数。
...
全部会话是最常用的观察口径。每次成功 Read 计入审计时，Server 会在同一个 SQLite 写事务中增量更新项目的已覆盖行数、已覆盖文件数、逐文件已覆盖行数和最大单行读取次数。因此 `auditcov coverage PROJECT_ID`、项目列表和 Web 的全部会话视图可以直接使用已物化结果，不需要反复扫描所有历史 Read 事件。

显式选择部分 session 时，Server 仍按这些 session 的成功读取区间现场计算并集和读取次数，保证筛选语义准确。即使调用方显式传入了项目当前的全部 session，Server 也会识别为全部会话并复用缓存。

已有 SQLite 数据库无需重新创建项目。升级后的 Server 第一次打开旧库时会自动执行一次历史回填，从现有成功 Read 事件重建全部会话汇总和逐文件峰值；后续启动会直接复用已完成的物化数据。
...
`project delete` 永久删除该项目在本地数据库中的快照、文件元数据、父子会话、读取事件和覆盖区间，因此必须显式传入 `--yes`。它不会删除代码仓或其中的任何文件。删除后 SQLite 文件大小可能不会立即缩小，释放的数据库页会由后续写入复用。
...
- 未传 `--session-id` 时统计项目全部会话；默认读取实时维护的轻量 summary，不构造完整文件树。
- `--tree` 请求完整目录/文件覆盖树；`--json` 只切换为 JSON 输出，不再隐式请求目录树。
- `--session-id` 可以重复，结果是这些精确会话覆盖区间的并集；选择父会话不会自动包含子会话。部分会话结果由 Server 现场计算。
- `--no-sessions` 明确选择空会话集合，分母不变、分子为零。
- 数据命令都支持 `--json`；使用 `coverage --tree --json` 时会包含完整目录树，大型项目的输出可能较大。
...
如果准备删除后立刻用相同路径重建项目，应先停止相关 Agent 或等待在途 Read 完成，避免旧调用的延迟 after 事件进入新项目。
...
- **Project list**：选择不同代码仓；Delete 按钮确认后删除该项目的全部 AuditCov 数据，但不删除源码文件。
- **All/None**：选择全部会话或清空选择。
- **Session tree**：按 Agent 类型和原生会话 ID 展示父子关系。
- **Project snapshot**：默认浏览可展开的冻结源码目录树；折叠目录不会创建其后代 DOM，展开时才渲染下一级。文件名前显示最大单行读取次数，`0` 为红色，正数越大绿色越深。
- **Directory / All files**：切换目录树与全部文件列表；全部文件模式只排序一次并分批渲染，可点击 **Load more files** 继续加载，并可按最大单行读取次数从高到低或从低到高排列。
...
| `DELETE` | `/api/projects/{id}` | 删除项目及其全部快照、会话、读取和覆盖数据 |
| `GET` | `/api/projects/{id}/coverage-summary` | 返回不含目录树的轻量覆盖总览；全部会话复用物化数据，部分会话现场合并 |
| `GET` | `/api/projects/{id}/coverage` | 查询所选会话的目录/文件覆盖率；文件节点包含 `max_read_count` |

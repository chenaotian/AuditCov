...
项目运行时没有第三方 Python 依赖。在仓库根目录中可直接运行，也可以选择执行 `python -m pip install -e .` 安装命令行入口。
...
| 工具 | 用途 |
| --- | --- |
| `auditcov_read_file` | 从已创建项目读取完整源码行，并记录当前 Codex 任务的覆盖区间 |
...
- **File coverage**：显示选定文件的代码内容和逐行覆盖标记。
...
| 变量 | 作用 |
| --- | --- |
| `AUDITCOV_SERVER_URL` | Agent 适配器连接的 Server 地址，默认 `http://127.0.0.1:8765` |
...
| `GET` | `/api/projects/{id}/file` | 查询所选会话的逐行文件覆盖详情 |
